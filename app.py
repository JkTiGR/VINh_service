...
# Определяем базовую директорию и загружаем переменные окружения
...
# Создание Blueprint для маршрутов с префиксом /vin.com
...
def submit_order():
    """
    Создаёт новую запись в таблице Client.
    """
...
@login_required
def update_order(client_id):
    """
    Обновляет существующую запись (Client) по ID.
    """
    data = request.get_json() if request.is_json else request.form

    # Пытаемся найти запись с указанным ID
